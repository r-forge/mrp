...
\section{School Vouchers, Simple Model}
\label{sec:scho-vouch-simple}



<<pew-example,echo=FALSE,include=FALSE,eval=FALSE>>=
## 1) Republican vote
  ## load data
  data(pew)
...
\setkeys{Gin}{width=7in}
<<lattice-sp-vouchers-altcolors, fig=TRUE,png=TRUE,pdf=FALSE,width=10,height=2.5>>=
...
print(
      spplot(spmap.states, 
             ...
             #names.attr=paste(rep(plotframe.names$row,each=length(plotframe.names$col)),
