...
\setbeamerfont{frametitle}{family={\fontsize{14}{12}\fontspec{Gotham-Medium}} }
\setbeamercolor{frametitle}{fg=MidnightBlue}

\setbeamerfont{title}{family=\fontspec{Gotham-Bold}}
...
\item[spmap.states] A projected map object with state names,
  \textsc{fips} codes, and two-letter state abbreviations.
\end{description}
\end{frame}
...
\begin{frame}
  \frametitle{things}
  In this case, the poll data we have uses four instead of five
  categories for Figure~\ref{fig:firstmap} education. Below, we combine the top two levels of Table~\ref{tab:table-simplemodel} 
  census education into a survey-matching single “Graduated College”
  level. We also drop any states from the census that are not in our
  survey dataset.
\end{frame}
<<prepare-census>>=
...
\begin{figure}
  ...
  \label{fig:firstmap}
<<firstmap,fig=TRUE,pdf=FALSE,png=TRUE,eps=FALSE,width=8,height=4,resolution=144>>=
...
<<finalmap,fig=TRUE,pdf=FALSE,png=TRUE,eps=FALSE,width=6.5,height=6.5,echo=FALSE,resolution=144>>=
<<finalmapcode>>
@   
\end{figure}
