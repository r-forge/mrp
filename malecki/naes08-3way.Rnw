...
within(naes08, function(){
  levels(state) <- sub(".*\\(([a-z]{2})\\)$",
                       "\\U\\1", # "florida (fl)" to "FL"
                       levels(state.of.residency),perl=TRUE)
  age2 <- age
  age2[age2>900] <- NA
  age <- cut(age2, c(0,29,44,64,900),
             labels=c("18-29","30-44","45-64","65+"))
  race2 <- race
  race[race2=="american indian" | 
       race2=="mixed race"] <- "other"
  race[of.hispanic.descent=="yes"] <- "hispanic"
  race[race2=="don't know" | 
       race2=="no answer"] <- NA
  
})
daply(naes08, .(state.of.residency,age), sum(favor.same.sex.marriage.or.civil.unions==1)
