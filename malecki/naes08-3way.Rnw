...
flattenNway <- function(v,design.effect) {
  if(sum(is.na(v))>0) { 
    v <- c(N=0,
...
foo <- na.omit(naes08[sample(1:nrow(naes08),1000),{names(naes08) %in% c("race","age","income","state","gays")}])

bar <- daply(foo, .(age,race,income), makeNway, response="gays",weights=1)
## Do overall design effect for later effective N
subsN <- 1:{prod(dim(bar))/3}
subsD <- {length(subsN)+1}:{length(subsN)*2}
design.effect <- weighted.mean(bar[subsD],
                               bar[subsN],na.rm=TRUE)
## Ply back into a data.frame for the MR-step
baz <- adply(bar, .margins=(1:(length(attr(bar,"dim"))-1)), flattenNway,
             design.effect=design.effect)
foo$gayz <- factor(foo$gays,ordered=TRUE,levels=c(1,0),labels=c("Yes","No"))
zoo <- newMrp (foo$gayz, 
        foo[, c("age", "race", "income")])
@ 
