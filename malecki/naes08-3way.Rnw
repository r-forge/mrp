...
regions <- data.frame(state=levels(foo$state),region=NA)
...
foo <- na.omit(naes08[sample(1:nrow(naes08),10000),{names(naes08) %in% c("race","age","income","state","gays")}])
...
mrp.gays <- mrp(## a formula. LHS is a binary variable
                ...
                #mr= . ~ (1+z.age|region) + (1+z.age|state) + (1+z.age|income) + (1|age),
                ## logical. Go ahead and run the MR-step?
                run=TRUE
                )
                
mrp <- function(formula,
                poll, pop=NULL, use=NULL,
                add=NULL, mr=NULL, run=FALSE,
                ...) {
  ...
  if(length(data.merges)>0){
    for(d in 1:length(data.merges)){
      data <- merge(data,data.merges[[d]],all.x=TRUE,all.y=FALSE)
    }
  }
  ...
  if (!is.null(pop)) { ## set up and store population NWayData
    ...
    if(!(identical(sapply(poll[,mrp.varnames], levels), 
      ...
      warning("Non-conformable population array. Poststratification will not work unless factor levels are identical.")
  ...
  mrp <- new("mrp",
             poll=poll.nway,
             data=data,
             formula=formula(mr.formula)
             )
  if(!is.null(pop.nway)){
    mrp@population <- pop.nway
  }
  if (run==TRUE) {
    response <- getResponse(mrp)
    fit <- lmer(formula=as.formula(mr.formula,env=parent.frame(2)),
               data=data,
               family=quasibinomial(link="logit"))
    mrp@multilevelModel <- fit
    mrp@theta.hat <- getThetaHat(mrp)
