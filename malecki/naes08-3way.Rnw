...
## econ: tax rates a problem, favor cutting taxes or strengthening social security, federal gov should reduce top tax rate, flat tax, spend more on social security, invest social security in stock market, poverty a problem, reduce income differences, spend more on aid to mothers with young children, expend effort to eliminate business regulations;
## give school vouchers, restrict abortion, ban abortion, favor death penalty, favor gun licenses, restrict gun purchases, underpunished criminals a problem, immigration a problem, gays in military, stop job discrimination against gays, stop job discrimination against blacks, women, allow school prayer. 

vargrep1 <- c("tax", "social", "invest", "pover", "income" ,"equal" , "welfare", "trade", "regul")
vargrep2 <- c("vouch", "abor", "death","pun", "gun" ,"crim", "citiz","imm", "gay" ,"same", "discr", "pray", "env","oil")

lapply(vargrep1, function(w) {
  v <- grep(w, names(naes08))
  vtext <- names(naes08)[v]
  names(vtext) <- v
  return(vtext)
})
lapply(vargrep2, function(w) {
  v <- grep(w, names(naes08))
  vtext <- names(naes08)[v]
  names(vtext) <- v
  return(vtext)
...
naes.vars1 <- c(162, )
naes.vars2 <- c(232, 316, 282, 353, 311)
...
cen <- na.omit(census[, {
  names(census) %in% c("race","age","income","state", "weighted2004")}])
...
print(poststratify(mrp.gays),digits=2)
mrp.gays <- mr(mrp.gays, .~.+(1|region))
print(poststratify(mrp.gays),digits=2)
...
## NEED TO DO: the 'maps' state map as an sp object!

spplot.mrp <- function(formula, object, spmap=NULL, FID="state", exclude=NULL, add.list=NULL, ...) {
...
mapme <- spplot.mrp(state~race+age, mrp.gays, 
               spmap=spmap.states, FID="STATE",
               exclude=c("DC","HI","AK"),emptycolor="#00000044")
