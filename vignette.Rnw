...
We would like to separate the estimation data by poll, but obviously the population data are the same across polls. To remove a stratification dimension, subtract it from the initial formulaby adding this argument:
<<eval=FALSE>>=
...
print(spplot(mrp.statelevel, state ~ edu+age,
                   ...
                   add.settings=list(
                     ...
                     superpose.line=list(col=c("black","#00000066"),lwd=c(.5,1))
                     ),
                   colorkey=list(
...
<<finalmap,fig=TRUE,pdf=TRUE,eps=FALSE,width=10,height=8,echo=FALSE>>=
<<finalmapcode>>
@   
\end{figure}
