...
The running example is the same combined poll data and model in Kastellec's \href{http://www.princeton.edu/~jkastell/mrp_primer.html}{MRP Primer}, dealing with support for same-sex marriage in 2004--2005. A simple model using only a few group-level intercepts, is run and subject to \textsc{R CMD check}; in the “not run” section of the examples, the complete model with census poststratification, individual- and state-level predictors, and categorical interactions is shown.

\subsection{Data}
\label{sec:data}

The data for the example are the combined survey results in the “marriage.data” data.frame, and state-level predictors in “Statelevel” data.frame. Both of these are loaded by the command \textsc{data(samesexmarriage)}. The package includes several other pieces of data that are useful to US practitioners of MRP:
\begin{description}
\item[mrp.census] Census data with main data columns “weighted2000”, “weighted2004”, and “weighted2008”. See documentation for \textsc{mrp.census} for details on the demographic features in the file.
...
marriage.data <- within(marriage.data, {
    ...
    labels(race.wbh) <- c("White","Black","Hispanic"))
