...
A map of results poststratified by state with no further conditioning (that is, collapsed across the other strata) using the included map object and \texttt{spplot} defaults is trivial to produce:
\begin{figure}
  \centering
  \caption{Support for same-sex marraige from simple MRP model.}
  \label{fig:firstmap}
<<firstmap,fig=TRUE,pdf=TRUE,png=FALSE,width=8,height=4>>=
