...
\begin{figure}[b]
  \centering
  \caption{The final map shows support for same-sex marriage by age cohort and level of education, centered around the national poststratified average of \sexpr{poststratify(mrp.statelevel)*100}\%.}
  \label{fig:finalmap}
